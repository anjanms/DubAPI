...
function EventHandler(message, env, channel) {
    ...
    case events.chatMessage:
        ...
        if (message.user && data.username !== message.user.username && typeof data.username === 'string') {
            message.user.username = data.username;
        }

        if (message.user && message.user.muted && !this.mutedTriggerEvents) return;
        if (!this._.room.chat.add(utils.clone(message, {deep: true}))) return;
        break;
